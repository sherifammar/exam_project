...
class QuizController extends GetxController{
 ...
 //question variables
  ...
  final RxInt _sec = 15.obs;// USING OBS
...
// check correct answer or wrong answer
  ...
  //check if the question has been answered (press or not press)
  ...
  void nextQuestion() {
    if (_timer != null || _timer!.isActive) {
      stopTimer();// 1-IF TIMER = STOP TIME
    }

    if (pageController.page == _questionsList.length - 1) {
      Get.offAndToNamed(ResultScreen.routeName);
      
    } else {// 2-  IF  NUM OF PAGE == LENGTH OF LIST -1 AND TRANSFER TO RESULT PAGE
      _isPressed = false;
      pageController.nextPage(
          duration: const Duration(milliseconds: 500), curve: Curves.linear);

      startTimer();//3- RETURN TO START TIME & begin NEXT QUESITION
    }
  ...
  void resetAnswer() {
    for (var element in _questionsList) {
      _questionIsAnswerd.addAll({element.id: false}); // convert another option is un press
    }
    update();
  }
  ...
  void startTimer() {
    resetTimer(); // max sec = 15 sce ---void resetTimer() => _sec.value = maxSec;
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (_sec.value > 0) {
        _sec.value--; // to decrease 15 (max time)sec -1
      } else {
        stopTimer(); // stop time 
  ...
  void resetTimer() => _sec.value = maxSec;// resetimer = 15 sec

  void stopTimer() => _timer!.cancel();// stop time
